...
fun RecordEditInfoScreen(
    ...
    // 녹음 상태변수 (enum 상태값 받음)
    val isRecording by viewModel.recordingEditState.collectAsStateWithLifecycle()
    ...
    DisposableEffect(Unit) {
        onDispose {
            ...
            if (viewModel is FakeRecordEditInfoScreenViewModel) {
...
fun FifthLoginInfoScreenPreview() {
    SisoTheme {
        ...
        val fakeViewModel = FakeRecordEditInfoScreenViewModel(audioRecorder = fakeAudioRecorder)
        RecordEditInfoScreen(viewModel = fakeViewModel, {})
    }
}
