...
fun LocationEditInfoScreen(
    ...
    val locationTextFloat = if (location.isBlank())  0.845F else 1F
    ...
    Box(
                ...
                Box(
                    ...
                    Row(
                        ...
                        Text(
                            ...
                            text = location.ifBlank { "검색" },
                            style = SisoTypoTokens.Body2,
                            color = SisoColorTokens.Gray70,
                            textAlign = TextAlign.Start
                        )
                        Spacer(Modifier.size(if (location.isNotBlank())3.dp
                        else 16.dp))
            ...
            if (location.isNotBlank()) {
                Column(
                    ...
                    CommonActiveButton(
                        ...
                        viewModel.locationComplete(input = location)
                        {input->
                            // 완료후 화면을 내리는 네비
                            popBackStack(input)
                        }
                    ...
                    if (bottomLocation.name.isEmpty()) {
                        ...
                        bottomLocation.name.forEach {
                            LocationText(input = it) {
                                // 하위 지역 추가
                                viewModel.setLocation("$location $it")
                                // 하위 지역 모드 초기화
                                viewModel.setBottomLocation("")
                                // 바텀 내리기
                                bottomState = false
        ...
        LoadingOverlay(locationState is LocationState.Loading)
...
fun LoadingOverlay(
    isLoading: Boolean,
    indicatorSize: Dp = 64.dp // 여기서 크기 조절 가능
) {
    if (isLoading) {
        Box(
            modifier = Modifier
                .fillMaxSize()
                .background(SisoColorTokens.Black.copy(alpha = 0.5f)) // 반투명 검은 배경
                .pointerInput(Unit) {
            ...
            CircularProgressIndicator(
                modifier = Modifier
                    .size(indicatorSize) // 크기 지정
                    .align(Alignment.Center),
                color = SisoColorTokens.White,
                strokeWidth = 6.dp // 스피너 두께도 조절 가능
            )
        }
    }
}
